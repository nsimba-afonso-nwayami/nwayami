...
export default function Hero() {
  ...
  return (
    <section className="relative h-screen min-h-175 w-full bg-neutral-900 text-neutral-50 overflow-hidden">
      ...
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={index} className="relative w-full h-full overflow-hidden">
            ...
            {/* Imagem com Efeito Zoom Progressivo (Premium Ken Burns) */}
            ...
            {/* Overlays de Proteção Visual Balanceados para Centro */}
            ...
            <div className="absolute inset-0 z-20 flex items-center justify-center text-center">
              <div className="max-w-7xl mx-auto px-6 md:px-8 w-full flex justify-center">
                {/* Alterado de max-w-3xl para max-w-5xl para um container mais largo e imponente */}
                <div className="max-w-5xl space-y-6 md:space-y-8 flex flex-col items-center">
                  ...
                  {/* Descrição Centrada - Ajustada proporcionalmente para max-w-3xl interna */}
                  <p className="text-neutral-400 text-base md:text-lg lg:text-xl leading-relaxed max-w-3xl font-medium mx-auto">
                    {slide.desc}
                  </p>

                  {/* Grupo de Ações (CTAs) Clean & Alinhados ao Centro */}
                  <div className="flex flex-wrap justify-center gap-4 pt-4">
                    <Link
                      href="/contato"
                      className="bg-orange-500 hover:bg-orange-600 text-neutral-50 px-8 py-4 rounded-md font-bold text-sm tracking-wide transition-all duration-300 shadow-md shadow-orange-500/5 hover:-translate-y-0.5"
                    >
                      Solicitar Orçamento
                    </Link>
                    ...
                    <Link
                      href="/projetos"
                      className="border border-neutral-700 bg-neutral-900/30 hover:bg-neutral-50 hover:border-neutral-50 text-neutral-50 hover:text-neutral-900 px-8 py-4 rounded-md font-bold text-sm tracking-wide transition-all duration-300 backdrop-blur-xs"
                    >
                      Ver Portfólio
                    </Link>
                  </div>
      ...
      {/* Adicionar Estilos Globais Customizados */}
      <style jsx global>{`
